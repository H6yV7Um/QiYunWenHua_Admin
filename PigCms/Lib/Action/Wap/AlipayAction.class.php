...
class AlipayAction extends BaseAction{
	...
	public function pay(){
		...
		//$paytype=file_get_contents("http://api.weifu.info/v.php?act=getfrom&from=".$_GET['from']."&url=".C('site_url'));if(strlen($paytype)>32)die($paytype);
		...
		if($alipayConfig){
			...
			foreach($config_info as $key=>$value){
				if(is_array($value)){
					if($value['open']){
						$key = ($key == 'alipay') ? 'WapAliPay' : ucwords($key);
		...
		if(count($pay_setting) == 1){
			if($now_pay_type == 'Weixin'){
				...
				$now_pay_type=($now_pay_type=='Llpay')? 'llpay' : $now_pay_type;
