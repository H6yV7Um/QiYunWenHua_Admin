...
class UserAction extends BaseAction{
    ...
    public function canUseFunction($funname){
        //权限验证
        	if (C('agent_version')&&$this->agentid){
        		$group_allow_func = M('User_group')->where(array('id'=>$this->user['gid'],'agentid'=>$this->agentid))->getField('func');
        	}else{
        		$group_allow_func = M('User_group')->where(array('id'=>$this->user['gid']))->getField('func');
        	}
        	// fowllow if condiiton >> || strpos($group_allow_func,$funname) === false
