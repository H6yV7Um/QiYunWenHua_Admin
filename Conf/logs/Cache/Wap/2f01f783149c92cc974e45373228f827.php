...
<link href="<?php echo RES;?>/css/allcss/cate20_<?php echo ($tpl["color_id"]); ?>.css" rel="stylesheet" type="text/css" />
    ...
    <script type="text/javascript" src="<?php echo RES;?>/css/120/swipe.js"></script>
...
<span id="playbox" class="btn_music" onclick="playbox.init(this).play();"><audio autoplay="autoplay" src="<?php echo ($homeInfo["musicurl"]); ?>" loop id="audio"></audio></span><?php endif; ?>
    ...
    <div style="-webkit-transform:translate3d(0,0,0);">
        ...
        <div class="list_nav">
            <ul class="box">
            <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i < 5): ?><li>
                    ...
                    <div><span ><img src="<?php echo ($vo["img"]); ?>" class="icon-smile" style="width:60px;height:42px;"></span></div>
        ...
        <div>
            <ul class="list_ul">
                <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($i > 4): ?><li class="tbox" <?php if($i%2 != 1): ?>style="direction:rtl;"<?php endif; ?> >
                    <div>
                        <a href="<?php if($vo['url'] == ''): echo U('Wap/Index/lists',array('classid'=>$vo['id'],'token'=>$vo['token'])); else: echo (htmlspecialchars_decode($vo["url"])); endif; ?>">
                            <img src="<?php echo ($vo["img"]); ?>"></a>
                    </div>
                    <div>
                        <dl>
                            <dt>
                            ...
                            <?php
 if($vo['sub']){ foreach($vo['sub'] as $res){ if(!$res['url']){ $res['url']=U('Wap/Index/lists',array('classid'=>$res['id'],'token'=>$_GET['token'])); } echo '<dd><a href="'.$res['url'].'"><div class="font">'.$res['name'].'</div></a></dd>'; } } ?>
                            </dl>
                    </div>
                </li><?php endif; endforeach; endif; else: echo "" ;endif; ?>
