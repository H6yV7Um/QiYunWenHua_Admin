...
<?php if($ordersCount != 0): if(is_array($orders)): $i = 0; $__LIST__ = $orders;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$o): $mod = ($i % 2 );++$i;?><ul class="m-uc-order-li">
			<span info_link="<?php echo U('Store/myDetail',array('token'=>$token,'cartid'=>$o['id'],'wecha_id'=>$wecha_id, 'twid' => $twid));?>" onclick="order_jump($(this))">
				<li class="p-li">
					<?php if(is_array($o['productInfo'])): $i = 0; $__LIST__ = $o['productInfo'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><a href="<?php echo U('Store/product',array('token'=>$token,'id'=>$row['id'],'wecha_id'=>$wecha_id, 'twid' => $twid));?>">
				...
				<?php else: ?>
					<?php if($o['paymode'] == 1): ?><span style="color:green">微信支付</span>
					<?php elseif($o['paymode'] == 2): ?><span style="color:green"> 财付通支付</span>
					<?php elseif($o['paymode'] == 3): ?><span style="color:green">货到付款</span>
					<?php elseif($o['paymode'] == 4): ?><span style="color:green">会员卡支付</span>
					<?php elseif($o['paymode'] == 5): ?><span style="color:green">积分兑换</span>
					<?php else: ?><span style="color:green">货到付款</span><?php endif; endif; ?><i class="t"><?php echo (date("Y-m-d H:i:s",$o["time"])); ?></i></li>
				...
				<li>订单总额：￥<?php echo ($o["price"]); ?>
					<?php if(($o['paid'] == 0) AND ($alipayConfig['open'] == 1)): ?><a href="<?php echo U('Store/orderCart', array('token' => $token, 'wecha_id' => $wecha_id,'orid' => $o['id'], 'twid' => $twid));?>" class="pay-btn">立即付款</a><?php endif; ?>
				</li>
			</span>
	...
	<?php if($totalpage > 1) { ?>
		<div class="m-page uc-orderlist">
			...
			<?php else: ?>
				<div class="pg-pre"><a href="<?php echo U('Store/my',array('token'=>$token,'page'=>intval($page - 1),'wecha_id'=>$wecha_id, 'twid' => $twid));?>">上一页<i><em></em></i></a></div><?php endif; ?>
			...
			<?php else: ?>
				<div class="pg-next"><a href="<?php echo U('Store/my',array('token'=>$token,'page'=>intval($page + 1),'wecha_id'=>$wecha_id, 'twid' => $twid));?>">下一页<i><em></em></i></a></div><?php endif; ?>
		</div>
	<?php }else{} ?>
<?php else: ?>
