...
$(document).ready(function() {
	$(".com_pg-num").click(function(){
		...
		$.get("<?php echo U('Store/getcomment',array('token'=>$token,'pid'=>$product['id'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>" + '&page='+page, function(response){
...
function add_cart() {
	...
	$.ajax({
		url: "<?php echo U('Store/addProductToCart',array('token'=>$token,'id'=>$product['id'],'wecha_id'=>$_GET['wecha_id'], 'twid' => $twid));?>" + '&count='+count + '&did=' + did,
...
function QuickBuy() {
	...
	$.ajax({
		...
		success: function(data) {
			if(data){
				location.href = "<?php echo U('Store/cart',array('token' => $token,'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>";;
			} else {
				return floatNotify.simple('抱歉，您的请求不正确');
			}
		}
...
window.shareData = {  
            ...
            "timeLineLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'], 'twid' => $mytwid));?>",
            "sendFriendLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'], 'twid' => $mytwid));?>",
            "weiboLink": "<?php echo C('site_url') . U('Store/product',array('token' => $_GET['token'],'id'=>$product['id'], 'twid' => $mytwid));?>",
