...
<?php if(is_array($products)): $i = 0; $__LIST__ = $products;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i; if(empty($p['detail']) != true): if(is_array($p['detail'])): $i = 0; $__LIST__ = $p['detail'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><li>
				...
				<?php if($row['comment'] == 1): ?><a href="<?php echo U('Store/comment',array('token'=>$token,'detailid'=>$row['id'],'wecha_id'=>$wecha_id,'pid'=>$p['id'], 'cartid' => $cartid, 'twid' => $twid));?>" style="font-size: 1.4rem;color: #fff;bottom: -1px;right: -1px;background: #ff8a00;border: 1px solid #f26100;padding: 2px 14px;line-height: 24px;border-radius: 4px 0 4px 0;">评论</a><?php endif; ?>
				</span>
	...
	<?php else: ?>
		<li>
			...
			<?php if($p['comment'] == 1): ?><a href="<?php echo U('Store/comment',array('token'=>$token,'wecha_id'=>$wecha_id,'pid'=>$p['id'], 'cartid' => $cartid, 'twid' => $twid));?>" style="font-size: 1.4rem;color: #fff;bottom: -1px;right: -1px;background: #ff8a00;border: 1px solid #f26100;padding: 2px 14px;line-height: 24px;border-radius: 4px 0 4px 0;">评论</a><?php endif; ?>
			</span>
...
<?php if(empty($cartData['paytype']) != true): if($cartData['paytype'] == 'alipay'): ?><span style="color:green">支付宝</span>
	<?php elseif($cartData['paytype'] == 'weixin'): ?><span style="color:green">微信支付</span>
	<?php elseif($cartData['paytype'] == 'tenpay'): ?><span style="color:green">财付通[wap手机]</span>
	<?php elseif($cartData['paytype'] == 'tenpayComputer'): ?><span style="color:green">财付通[即时到帐]</span>
	<?php elseif($cartData['paytype'] == 'yeepay'): ?><span style="color:green">易宝支付</span>
	<?php elseif($cartData['paytype'] == 'allinpay'): ?><span style="color:green">通联支付</span>
	<?php elseif($cartData['paytype'] == 'daofu'): ?><span style="color:green">货到付款</span>
	<?php elseif($cartData['paytype'] == 'dianfu'): ?><span style="color:green">到店付款</span>
	<?php elseif($cartData['paytype'] == 'chinabank'): ?><span style="color:green">网银在线</span><?php endif; ?>
    ...
    confirm =floatNotify.confirm("确定要删除此订单吗？", "",
        function(t, n) {
            if(n==true){
                ...
                $.ajax({
                	type:"POST",
                	url: "<?php echo U('Store/cancelCart',array('token' => $token, 'cartid' => $cartid, 'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>",
                    dataType:"json",
                    success:function(data){
                        if(data.error_code == false){
                            floatNotify.simple('订单取消成功');
                            setTimeout("location.href='<?php echo U('Store/my',array('token' => $token, 'wecha_id' => $_GET['wecha_id'], 'twid' => $twid));?>'",1200);  
                        }else{
...
window.shareData = {  
            ...
            "moduleID":"0",
            "imgUrl": "", 
            "timeLineLink": "<?php echo C('site_url') . U('Store/myDetail',array('token' => $_GET['token'], 'twid' => $mytwid));?>",
            "sendFriendLink": "<?php echo C('site_url') . U('Store/myDetail',array('token' => $_GET['token'], 'twid' => $mytwid));?>",
            "weiboLink": "<?php echo C('site_url') . U('Store/myDetail',array('token' => $_GET['token'], 'twid' => $mytwid));?>",
